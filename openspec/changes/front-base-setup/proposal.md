...
- Настройка нативных git-хуков через `core.hooksPath`: pre-commit → lint-staged, pre-push → test + check-types + build.
...
- `git-hooks`: Автоматические проверки кода через нативные git hooks (core.hooksPath)
...
- **Зависимости**: React 19, Vite 8, TypeScript 6, Vitest 5, ESLint 10, Tailwind CSS 4, lint-staged 17, Stylelint 17, Prettier 3.
- **Файлы**: `package.json`, `vite.config.ts`, `tsconfig.json`, `src/main.tsx`, `src/App.tsx`, `src/index.css`, `src/vite-env.d.ts`, `src/test/setup.ts`, `src/App.test.tsx`, `.gitignore`, `README.md`, конфиги (`.mjs`), `.githooks/`, `.npmrc`, `.nvmrc`, `.env.example`, `pnpm-workspace.yaml`.
