...
Фронтенд реализует пользовательский интерфейс облегчённого аналога GitLab Duo Code Review. Основной сценарий — просмотр прогона ревью: его статуса, изменений в коде, контекста вокруг изменений и inline-замечаний AI-ревьюера.
...
Такое разделение соответствует сценарию GitLab Duo: diff остаётся центральным источником правок, локальный контекст раскрывается по запросу, а замечания привязаны к конкретным строкам изменённого кода.
